<script lang="ts">
	...
	const insertThread = async () => {
		...
		let threadsData = await data.supabase.from('threads').select().match({thread_id: thread?.id}).maybeSingle();
		
		if(!threadsData.data){
			await data.supabase.from('threads').insert({ user_id: userID, thread_id: thread?.id, assistant_id: assistantWithPatient?.id});
		}
	...
	const createThread = async () => {
		...
		let threadsData = await data.supabase.from('threads').select('thread_id').match({user_id: userID, assistant_id: assistantWithPatient?.id}).maybeSingle();
